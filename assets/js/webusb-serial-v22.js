...
(function (global) {
  ...
  class WebUsbSerialPort {
    ...
    getDebugInfo() {
      const configuration = this.device && this.device.configuration;
      return {
        driver: this.driver,
        configurationValue: configuration ? configuration.configurationValue : null,
        controlInterfaceNumber: this.controlInterfaceNumber,
        dataInterfaceNumber: this.interfaceNumber,
        inEndpointNumber: this.inEndpoint ? this.inEndpoint.endpointNumber : null,
        outEndpointNumber: this.outEndpoint ? this.outEndpoint.endpointNumber : null,
        packetSize: this.packetSize,
        opened: !!(this.device && this.device.opened)
      };
    }

    async writeDirect(chunk) {
      const bytes = chunk instanceof Uint8Array ? chunk : new Uint8Array(chunk);
      if (!this.opened || !this.device || !this.device.opened) {
        throw new DOMException('El puerto WebUSB no está abierto.', 'InvalidStateError');
      }
      if (!bytes.byteLength) return { status: 'ok', bytesWritten: 0 };
      const result = await this._writePacket(bytes);
      const bytesWritten = Number.isFinite(Number(result.bytesWritten))
        ? Number(result.bytesWritten) : bytes.byteLength;
      if (bytesWritten !== bytes.byteLength) {
        throw new Error('USB OUT incompleto: ' + bytesWritten + ' de ' + bytes.byteLength + ' bytes.');
      }
      return { status: result.status || 'ok', bytesWritten };
    }

    async readDirect(readSize) {
      if (!this.opened || !this.device || !this.device.opened) {
        throw new DOMException('El puerto WebUSB no está abierto.', 'InvalidStateError');
      }
      const size = Math.max(this.packetSize || 64, Number(readSize) || this.packetSize || 64);
      const result = await this._readPacket(size);
      if (!result.data || !result.data.byteLength) return new Uint8Array(0);
      const raw = new Uint8Array(result.data.buffer, result.data.byteOffset, result.data.byteLength);
      return this._transformIncoming(new Uint8Array(raw));
    }
