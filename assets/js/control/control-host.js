(function(){
  'use strict';
  if(window.__SIMUPLC_CONTROL_HOST_V22__) return;
  window.__SIMUPLC_CONTROL_HOST_V22__=true;
  ...
  let runtimeOn=false, pollTimer=0, lastInputs={}, lastElectricalInputs={};
  ...
  function fbdState(){
    ...
    try{fbdNodes().forEach(function(n){if(!n||!n.name)return;const tag=String(n.name).trim().toUpperCase();if(!tag)return;if(n.type==='input')inputs[tag]=!!(n.el&&n.el.classList.contains('active'));if(n.type==='output'||n.type==='M')outputs[tag]=!!n.value;});}catch(_){}
    return {inputs:inputs,outputs:outputs,simulationOn:fbdRunning(),receivedAt:Date.now()};
  }

  function forwardLadderState(st){ if(!runtimeOn||selectedSource!=='ladder')return; postControl({type:'SIMUPLC_CONTROL_RUNTIME_STATE',source:'ladder',state:st||{inputs:{},outputs:{},simulationOn:false}}); }
  ...
  function runtimeStart(source){
    ...
    runtimeOn=true;lastInputs={};lastElectricalInputs={};announceLink();
    if(selectedSource==='fbd'){
      ladderPost('stop',{});fbdStart();
      setTimeout(function(){if(runtimeOn&&selectedSource==='fbd'){fbdSetInputs(lastElectricalInputs);forwardFbdState();}},35);
    }else{
      fbdStop();ladderPost('start',{});
      setTimeout(function(){if(runtimeOn&&selectedSource==='ladder'){lastInputs=electricalToLadderActuation(lastElectricalInputs);ladderPost('setInputs',{values:lastInputs});ladderPost('getState',{});}},30);
    }
    startPoll();
  }
  function runtimeStop(source){
    if(source&&normalizeSource(source)!==selectedSource){/* el origen antiguo no cambia la selección */}
    runtimeOn=false;stopPoll();lastInputs={};lastElectricalInputs={};
    try{ladderPost('stop',{});}catch(_){} try{fbdStop();}catch(_){}
    postControl({type:'SIMUPLC_CONTROL_RUNTIME_STATE',source:selectedSource,state:{inputs:{},outputs:{},simulationOn:false}});
  }
  ...
  window.SimuPLCControlHost = {
    ...
    runtime:{start:runtimeStart,stop:runtimeStop,setInputs:runtimeInputs,setSource:setSource,getSource:function(){return selectedSource;},isRunning:function(){return runtimeOn;}}
  };
})();
