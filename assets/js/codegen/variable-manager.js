(function (global) {
  ...
  const VERSION = '1.7.1-v16-vars-v24.1';
  ...
  function normalizeFunctionRole(value) {
    ...
    if (role === 'safety' || role === 'seguridad') return 'safety';
    if (role === 'process' || role === 'proceso') return 'process';
    if (role === 'command' || role === 'mando' || role === 'control') return 'command';
    return '';
  }

  function functionRoleLabel(value) {
    const role = normalizeFunctionRole(value);
    return role === 'safety' ? 'Seguridad' : (role === 'process' ? 'Proceso' : (role === 'command' ? 'Mando' : '—'));
  }

  function inferFunctionRole(row, meta) {
    if (!row || row.kind !== 'digitalInput') return '';
    const explicit = normalizeFunctionRole(meta && (meta.functionRole || meta.role || meta.function));
    if (explicit) return explicit;
    const text = [
      row.label,
      meta && meta.symbolicName,
      meta && meta.name,
      meta && meta.description,
      row.symbolicSuggestions && row.symbolicSuggestions.join(' '),
      row.descriptions && row.descriptions.join(' '),
      row.usages && row.usages.join(' ')
    ].filter(Boolean).join(' ');
    if (/STOP|PARADA|EMERGENCIA|E-?STOP|GUARDAMOTOR|PROTECCI(?:O|Ó)N|T[EÉ]RMICO|SOBRECARGA|FALLA|SAFETY/i.test(text)) return 'safety';
    if (/SENSOR|NIVEL|PUERTA|FINAL(?: DE)? CARRERA|FOTO|PRESENCIA|PROCESO|ELECTRONIVEL|CILINDRO|TANQUE|TEMPERATURA|PRESI(?:O|Ó)N|CAUDAL/i.test(text)) return 'process';
    return 'command';
  }

  function roleOptions(row) {
    if (!row || row.kind !== 'digitalInput') return '<span class="vm-no-pin">—</span>';
    const current = normalizeFunctionRole(row.functionRole) || 'command';
    const options = [
      ['command','MANDO'],
      ['process','PROCESO'],
      ['safety','SEGURIDAD']
    ];
    return '<select class="vm-role" data-variable="' + escapeHTML(row.label) + '">' + options.map(function (item) {
  ...
  function sortPLC(a, b) {
    ...
    let ln = Number((left.match(/\d+/) || [999999])[0]);
    let rn = Number((right.match(/\d+/) || [999999])[0]);
    if(global.SimuPLCPLCProfile && (lp==='I'||lp==='Q') && lp===rp){
      const la=global.SimuPLCPLCProfile.addressOrdinal(left), ra=global.SimuPLCPLCProfile.addressOrdinal(right);
      if(Number.isFinite(la)) ln=la;
      if(Number.isFinite(ra)) rn=ra;
    }
  ...
  function persistWorkingState() {
    currentRows.forEach(function (row) {
      ...
      if (!workingMeta.variables[row.label]) workingMeta.variables[row.label] = {};
      if (safeText(row.symbolicName).trim()) workingMeta.variables[row.label].symbolicName = safeText(row.symbolicName).trim();
      if (safeText(row.description).trim()) workingMeta.variables[row.label].description = safeText(row.description).trim();
      if (row.kind === 'digitalInput') workingMeta.variables[row.label].functionRole = normalizeFunctionRole(row.functionRole) || 'command';
    });
  ...
  function splitVariableRefs(value) {
    return safeText(value).split(/[;,\s]+/).map(upper).filter(Boolean);
  }

  function inferKindFromLabel(label, fallbackKind) {
    const key = upper(label);
    if (/^I\d+(?:\.\d+)?$/.test(key)) return 'digitalInput';
    if (/^AI\d+$/.test(key)) return 'analogInput';
    if (/^Q\d+(?:\.\d+)?$/.test(key)) return 'digitalOutput';
    if (/^PWM\d+$/.test(key)) return 'pwmOutput';
    if (/^AO\d+$/.test(key)) return 'analogOutput';
    if (/^M\d+$/.test(key)) return 'memory';
    if (/^(?:T|TON|TOFF)\d+$/.test(key)) return 'timer';
    if (/^(?:C|CTU|CNT)\d+$/.test(key)) return 'counter';
    if (/^CONST\d+$/.test(key)) return 'analogConstant';
    if (/^PID\d+$/.test(key)) return 'internalAnalog';
    if (/^B\d+$/.test(key)) return 'internalDigital';
    return fallbackKind || null;
  }

  function hmiValue(element, tag, kind) {
    if (!element) return '—';
    if (kind === 'analogInput' || kind === 'analogOutput' || kind === 'pwmOutput' || kind === 'internalAnalog') {
      const value = Number(element.analogValue);
      if (Number.isFinite(value)) {
        const decimals = Number.isFinite(Number(element.analogDecimals)) ? Number(element.analogDecimals) : 1;
        const unit = safeText(element.analogUnit || '').trim();
        return value.toFixed(Math.max(0, Math.min(4, decimals))) + (unit ? ' ' + unit : '');
      }
    }
    if ((kind === 'digitalInput' || kind === 'digitalOutput') && typeof element.latched === 'boolean') return element.latched ? '1' : '0';
    return '—';
  }
  ...
  function addVariable(store, data) {
    ...
    const symbolic = safeText(data.symbolicSuggestion).trim();
    if (symbolic && upper(symbolic) !== label && row.symbolicSuggestions.indexOf(symbolic) < 0) row.symbolicSuggestions.push(symbolic);
    const usage = safeText(data.usage).trim();
    if (usage && row.usages.indexOf(usage) < 0) row.usages.push(usage);
  }
  ...
  function collectFBD(store, state) {
    ...
    (state && state.nodes || []).forEach(function (node) {
      ...
      addVariable(store, {
        ...
        symbolicSuggestion: safeText(node && (node.symbolicName || node.alias || '')).trim(),
        usage: 'FBD' + (firstDescription(node) ? ' · ' + firstDescription(node) : ''),
  ...
  function collectLadder(store, model) {
    ...
    flattenLadder(model).forEach(function (element) {
      ...
      addVariable(store, {
        ...
        symbolicSuggestion: safeText(element && (element.symbolicName || element.alias || '')).trim(),
        usage: 'Ladder' + (firstDescription(element) ? ' · ' + firstDescription(element) : ''),
  ...
  function collectHMI(store, project) {
    const elements = project && Array.isArray(project.elements) ? project.elements : [];
    elements.forEach(function (element) {
      if (!element) return;
      const elementLabel = safeText(element.label || element.name || '').trim();
      ['tag','tag2','tag3','tag4'].forEach(function (field) {
        splitVariableRefs(element[field]).forEach(function (tag) {
          const kind = inferKindFromLabel(tag, null);
          if (!kind) return;
          addVariable(store, {
            label: tag,
            kind: kind,
            ...
            symbolicSuggestion: elementLabel,
            usage: 'HMI' + (elementLabel ? ' · ' + elementLabel : '') + (field !== 'tag' ? ' · ' + field.toUpperCase() : ''),
            value: hmiValue(element, tag, kind)
          });
        });
      });
    });
  ...
  async function collectVariables() {
    ...
    try {
      if (global.SimuPLCHMI && typeof global.SimuPLCHMI.getProject === 'function') hmi = global.SimuPLCHMI.getProject() || hmi;
    } catch (_) {}
    collectFBD(store, fbd);
    collectLadder(store, ladder);
    collectHMI(store, hmi);
    ...
    const rows = Array.from(store.values()).map(function (row) {
      ...
      const storedSymbolicName = safeText(meta.symbolicName || meta.name).trim();
      row.origin = Array.from(row.origins).sort().join(' + ');
      row.description = storedDescription || row.descriptions[0] || '';
      row.symbolicName = storedSymbolicName || row.symbolicSuggestions[0] || '';
      row.functionRole = inferFunctionRole(row, meta);
      row.usage = row.usages.join(' | ');
      row.value = row.values.length ? row.values.join(' / ') : '—';
      delete row.origins;
      delete row.descriptions;
      delete row.values;
      delete row.symbolicSuggestions;
      delete row.usages;
      return row;
    });
  ...
  function validateRows(boardId) {
    ...
    const symbolicUsage = new Map();
    currentRows.forEach(function (row) {
      const name = safeText(row.symbolicName).trim().toUpperCase();
      if (!name) return;
      if (!symbolicUsage.has(name)) symbolicUsage.set(name, []);
      symbolicUsage.get(name).push(row.label);
    });
    symbolicUsage.forEach(function (labels, name) {
      if (labels.length < 2) return;
      labels.forEach(function (label) {
        states[label].warnings.push('Nombre simbólico repetido “' + name + '” con ' + labels.filter(function (other) { return other !== label; }).join(', ') + '.');
        if (states[label].status !== 'error') states[label].status = 'warning';
      });
    });
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .vm-label{font-weight:1000;font-size:13px;color:#102a43}.vm-kind{font-weight:800;color:#36586b}.vm-role-label{display:block;margin:4px 0 2px;font:800 8px Arial;color:#64748b;letter-spacing:.35px}.vm-role{width:100%;height:30px;border:1px solid #b9cfdd;border-radius:7px;background:#fff;padding:0 6px;font:900 9px Arial;color:#0f3b5a}.vm-origin{display:inline-flex;border-radius:999px;background:#e9f5fb;color:#11607d;padding:4px 7px;font-size:10px;font-weight:900}
      .vm-name,.vm-desc{width:100%;height:34px;border:1px solid #c5d6e0;border-radius:8px;padding:0 9px;background:#fff;color:#0f2940}.vm-name{min-width:130px;font-weight:900}.vm-desc{min-width:180px}.vm-pin{width:150px;height:34px;border:1px solid #b9cfdd;border-radius:8px;background:#fff;padding:0 8px;font-weight:800}.vm-pin.invalid{border-color:#d92d20;background:#fff5f4}.vm-no-pin{font-weight:800;color:#617683}.vm-row-message{max-width:220px;font-size:10px;line-height:1.35;color:#667d89}.vm-row-message.error{color:#b42318;font-weight:800}.vm-row-message.warning{color:#9a6700;font-weight:800}
      ...
      @media(max-width:760px){
        ...
        .vm-table th:nth-child(3),.vm-table td:nth-child(3){width:105px}
        .vm-table th:nth-child(6),.vm-table td:nth-child(6){width:auto}
        .vm-table th:nth-child(8),.vm-table td:nth-child(8){width:104px}
        .vm-table th:nth-child(4),.vm-table td:nth-child(4),.vm-table th:nth-child(5),.vm-table td:nth-child(5),.vm-table th:nth-child(7),.vm-table td:nth-child(7),.vm-table th:nth-child(9),.vm-table td:nth-child(9){display:none}
        .vm-table td:before{content:none}.vm-label{font-size:11px}.vm-status-dot{width:22px;height:22px}.vm-name,.vm-desc,.vm-pin,.vm-role{min-width:0;width:100%;height:31px;font-size:10px}.vm-row-message{max-width:none}
      ...
      @media(min-width:520px) and (max-width:760px){
        .vm-table{min-width:980px}.vm-table th:nth-child(4),.vm-table td:nth-child(4),.vm-table th:nth-child(5),.vm-table td:nth-child(5),.vm-table th:nth-child(7),.vm-table td:nth-child(7),.vm-table th:nth-child(9),.vm-table td:nth-child(9){display:table-cell}
        .vm-table th:nth-child(4),.vm-table td:nth-child(4){width:125px}.vm-table th:nth-child(5),.vm-table td:nth-child(5){width:105px}.vm-table th:nth-child(7),.vm-table td:nth-child(7){width:76px}.vm-table th:nth-child(9),.vm-table td:nth-child(9){width:170px}
      }
      html.theme-dark #variableManagerCard{background:linear-gradient(180deg,#111827,#0f172a);color:#e5eef8;border-color:#334155}
      html.theme-dark .vm-head,html.theme-dark .vm-toolbar,html.theme-dark .vm-filters,html.theme-dark .vm-footer{background:#111827;border-color:#334155}
      html.theme-dark .vm-head{background:linear-gradient(135deg,#0f2537,#111827)}
      html.theme-dark .vm-head p,html.theme-dark .vm-field label,html.theme-dark .vm-footer-note{color:#9fb3c8}
      html.theme-dark .vm-summary,html.theme-dark .vm-table-wrap{background:#0f172a;border-color:#334155}
      html.theme-dark .vm-stat{background:#111827;border-color:#334155}html.theme-dark .vm-stat span{color:#9fb3c8}
      html.theme-dark .vm-table th{background:#172637;color:#cfe8f6;border-color:#334155}html.theme-dark .vm-table td{border-color:#263649;color:#e5eef8}html.theme-dark .vm-table tr:hover td{background:#152235}
      html.theme-dark .vm-field select,html.theme-dark .vm-search,html.theme-dark .vm-name,html.theme-dark .vm-desc,html.theme-dark .vm-pin,html.theme-dark .vm-role,html.theme-dark .vm-btn,html.theme-dark .vm-filter,html.theme-dark .vm-close{background:#0f172a;color:#e5eef8;border-color:#475569}
      html.theme-dark .vm-label{color:#f8fafc}html.theme-dark .vm-kind{color:#cbd5e1}html.theme-dark .vm-role-label{color:#94a3b8}html.theme-dark .vm-origin{background:#123149;color:#7dd3fc}html.theme-dark .vm-no-pin,html.theme-dark .vm-row-message{color:#aab9c8}
    ...
    modal.innerHTML = `
      <section id="variableManagerCard" role="dialog" aria-modal="true" aria-labelledby="vmTitle">
        <header class="vm-head">
          <div class="vm-head-main"><h2 id="vmTitle">Tabla profesional de variables</h2><p>Centraliza las variables de FBD, Ladder y HMI. Define nombre simbólico, descripción y pin físico una sola vez; esta tabla queda preparada como fuente común para CONTROL y CÓDIGO MCU.</p></div>
        ...
        <div class="vm-toolbar">
          ...
          <input id="vmSearch" class="vm-search" type="search" placeholder="Buscar variable, nombre, descripción o editor..." />
        ...
        <div class="vm-filters" id="vmFilters">
          ...
          <button class="vm-filter" data-filter="fbd" type="button">FBD</button>
          <button class="vm-filter" data-filter="ladder" type="button">Ladder</button>
          <button class="vm-filter" data-filter="hmi" type="button">HMI</button>
          <button class="vm-filter" data-filter="errors" type="button">Con errores</button>
        </div>
        <div class="vm-table-wrap">
          <table class="vm-table">
            <thead><tr><th>Estado</th><th>Variable</th><th>Nombre</th><th>Tipo</th><th>Usado en</th><th>Descripción</th><th>Valor</th><th>Pin físico</th><th>Diagnóstico</th></tr></thead>
  ...
  function filteredRows(validation) {
    ...
    return currentRows.filter(function (row) {
      ...
      if (currentFilter === 'fbd' && row.origin.indexOf('FBD') < 0) return false;
      if (currentFilter === 'ladder' && row.origin.indexOf('Ladder') < 0) return false;
      if (currentFilter === 'hmi' && row.origin.indexOf('HMI') < 0) return false;
      if (query) {
        const haystack = [row.label, row.symbolicName, row.typeTitle, functionRoleLabel(row.functionRole), row.origin, row.description, row.usage].join(' ').toLowerCase();
  ...
  function renderTable() {
    ...
    if (!rows.length) {
      body.innerHTML = '<tr><td colspan="9" style="padding:24px;text-align:center;color:#6b7f8b">No hay variables para este filtro.</td></tr>';
      return;
    }
    body.innerHTML = rows.map(function (row) {
      ...
      return '<tr data-variable="' + escapeHTML(row.label) + '">' +
        ...
        '<td data-label="Nombre"><input class="vm-name" data-variable="' + escapeHTML(row.label) + '" value="' + escapeHTML(row.symbolicName) + '" placeholder="Ej.: START, KM1" /></td>' +
        '<td data-label="Tipo"><span class="vm-kind">' + escapeHTML(row.typeTitle) + '</span>' + (row.kind === 'digitalInput' ? '<small class="vm-role-label">FUNCIÓN</small>' + roleOptions(row) : '') + '</td>' +
        '<td data-label="Usado en"><span class="vm-origin" title="' + escapeHTML(row.usage || row.origin) + '">' + escapeHTML(row.origin) + '</span></td>' +
        '<td data-label="Descripción"><input class="vm-desc" data-variable="' + escapeHTML(row.label) + '" value="' + escapeHTML(row.description) + '" placeholder="Ej.: Pulsador de marcha" /></td>' +
        '<td data-label="Valor"><span>' + escapeHTML(row.value) + '</span></td>' +
        '<td data-label="Pin físico">' + (row.physical ? '<select class="vm-pin ' + (state.errors.length ? 'invalid' : '') + '" data-group="' + row.group + '" data-variable="' + escapeHTML(row.label) + '">' + pinOptions(row, profile, current) + '</select>' : '<span class="vm-no-pin">No requerido</span>') + '</td>' +
  ...
  function bindModalEvents() {
    ...
    document.getElementById('vmBody').addEventListener('input', function (event) {
      const input = event.target.closest('.vm-desc,.vm-name');
      if (!input) return;
      ...
      if (input.classList.contains('vm-name')) {
        workingMeta.variables[label].symbolicName = input.value;
        if (row) row.symbolicName = input.value;
      } else {
    ...
    document.getElementById('vmBody').addEventListener('change', function (event) {
      const roleSelect = event.target.closest('.vm-role');
      if (roleSelect) {
        const label = upper(roleSelect.dataset.variable);
        const role = normalizeFunctionRole(roleSelect.value) || 'command';
        if (!workingMeta.variables[label]) workingMeta.variables[label] = {};
        workingMeta.variables[label].functionRole = role;
        const row = currentRows.find(function (item) { return item.label === label; });
        if (row) row.functionRole = role;
        dirty = true;
        return;
      }
    ...
    document.getElementById('vmSave').addEventListener('click', function () {
      persistWorkingState();
      renderTable();
      try { global.dispatchEvent(new CustomEvent('simuplc-variable-catalog-changed', { detail: { metadata: clone(workingMeta), rows: clone(currentRows) } })); } catch (_) {}
      showToast(lastValidation && lastValidation.ok ? 'Variables, funciones, nombres y pines guardados' : 'Guardado; aún hay errores por corregir');
    });
  ...
  function exportCSV() {
    ...
    const profile = getProfile(currentBoard);
    const map = mapForBoard(currentBoard);
    const lines = [['Variable','Nombre','Tipo','Función','Usado en','Descripción','Valor','Placa','Pin físico','Estado','Diagnóstico'].map(csvCell).join(',')];
    currentRows.forEach(function (row) {
      ...
      lines.push([
        row.label, row.symbolicName, row.typeTitle, functionRoleLabel(row.functionRole), row.origin, row.description, row.value, profile.name, pin,
  ...
  async function open() {
    ...
    if (message) { message.className = 'vm-message ok'; message.textContent = 'Leyendo variables de FBD, Ladder y HMI...'; }
    await refresh();
  }
  ...
  function exportConfig() {
    return {
      ...
      plcProfileId: global.SimuPLCPLCProfile && global.SimuPLCPLCProfile.getId ? global.SimuPLCPLCProfile.getId() : 'classic',
  ...
  function importConfig(config) {
    ...
    if (config.plcProfileId && global.SimuPLCPLCProfile && typeof global.SimuPLCPLCProfile.set === 'function') {
      global.SimuPLCPLCProfile.set(config.plcProfileId, { source: 'hardware-import', migrate: false, migrateStorage: false });
    }
    ...
    try { global.dispatchEvent(new CustomEvent('simuplc-variable-catalog-changed', { detail: { metadata: loadJSON(META_KEY, { variables: {} }) } })); } catch (_) {}
    return true;
  }
  ...
  global.SimuPLCVariableManager = Object.freeze({
    ...
    getSymbolicName: function (label) {
      const meta = loadJSON(META_KEY, { variables: {} });
      const item = meta.variables && meta.variables[upper(label)];
      return safeText(item && (item.symbolicName || item.name)).trim();
    },
    getDisplayName: function (label) {
      const key = upper(label);
      const meta = loadJSON(META_KEY, { variables: {} });
      const item = meta.variables && meta.variables[key];
      const name = safeText(item && (item.symbolicName || item.name)).trim();
      return name ? name + ' · ' + key : key;
    },
    setSymbolicName: function (label, name) {
      const key = upper(label);
      if (!key) return false;
      const meta = loadJSON(META_KEY, { version: VERSION, variables: {} });
      if (!meta.variables || typeof meta.variables !== 'object') meta.variables = {};
      if (!meta.variables[key]) meta.variables[key] = {};
      meta.variables[key].symbolicName = safeText(name).trim();
      meta.version = VERSION;
      meta.updatedAt = new Date().toISOString();
      saveJSON(META_KEY, meta);
      try { global.dispatchEvent(new CustomEvent('simuplc-variable-catalog-changed', { detail: { metadata: clone(meta) } })); } catch (_) {}
      return true;
    },
    getFunctionRole: function (label) {
      const key = upper(label);
      const meta = loadJSON(META_KEY, { variables: {} });
      const item = meta.variables && meta.variables[key];
      return normalizeFunctionRole(item && (item.functionRole || item.role || item.function));
    },
    setFunctionRole: function (label, role) {
      const key = upper(label);
      const normalized = normalizeFunctionRole(role);
      if (!key || !normalized) return false;
      const meta = loadJSON(META_KEY, { version: VERSION, variables: {} });
      if (!meta.variables || typeof meta.variables !== 'object') meta.variables = {};
      if (!meta.variables[key]) meta.variables[key] = {};
      meta.variables[key].functionRole = normalized;
      meta.version = VERSION;
      meta.updatedAt = new Date().toISOString();
      saveJSON(META_KEY, meta);
      try { global.dispatchEvent(new CustomEvent('simuplc-variable-catalog-changed', { detail: { metadata: clone(meta) } })); } catch (_) {}
    ...
    getCatalog: async function () { loadWorkingState(); currentRows = await collectVariables(); return clone(currentRows); },
