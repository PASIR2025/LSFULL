(function (global) {
  ...
  const VERSION = '1.8.2-autonomous-hmi-v24.2';
  ...
  const ANALOG_TYPES = new Set(['analog_constant', 'analog_input', 'scale', 'pid', 'split_range']);
  ...
  function variableMetadata(label) {
    const key=String(label||'').trim().toUpperCase();
    try { const vm=global.SimuPLCVariableManager,meta=vm&&vm.getMetadata?vm.getMetadata():null;return meta&&meta.variables&&meta.variables[key]?meta.variables[key]:null; } catch (_) { return null; }
  }

  function metadataFunctionRole(label) {
    const item=variableMetadata(label),role=String(item&&(item.functionRole||item.role||item.function)||'').trim().toLowerCase();
  ...
  function hmiInputElement(label) {
    const key=String(label||'').trim().toUpperCase();
    try {
      const p=global.SimuPLCHMI&&typeof global.SimuPLCHMI.getProject==='function'?global.SimuPLCHMI.getProject():null,els=p&&Array.isArray(p.elements)?p.elements:[];
      return els.find(function(el){return el&&[el.tag,el.tag2,el.tag3,el.tag4].some(function(v){return String(v||'').trim().toUpperCase()===key;});})||null;
    } catch (_) { return null; }
  }

  function hmiContactTypeForTag(label) {
    const key=String(label||'').trim().toUpperCase(),el=hmiInputElement(key);
    if(!el)return '';
    if(el.type==='tank'){
      if(String(el.tag||'').trim().toUpperCase()===key)return el.lowSensorContactType==='nc'?'nc':'no';
      if(String(el.tag2||'').trim().toUpperCase()===key)return el.highSensorContactType==='nc'?'nc':'no';
    }
    return String(el.contactType||'').toLowerCase();
  }

  function likelyNcInput(label, source) {
    const item=variableMetadata(label),metaText=item?[item.symbolicName,item.name,item.description].filter(Boolean).join(' '):'';
    const text = String(label || '') + ' ' + String(source && (source.label || source.name || source.description || '') || '') + ' ' + metaText;
    ...
    if(mode === 'nc' || /(?:^|[-_ ])nc(?:$|[-_ ])|STOP|PARADA|EMERGENCIA|E-?STOP|GUARDAMOTOR|PROTECCI(?:O|Ó)N|T[EÉ]RMICO|SOBRECARGA/i.test(text))return true;
    const hmi=hmiInputElement(label),hmiContact=hmiContactTypeForTag(label);
    return !!(hmi&&(hmi.type==='estop'||hmiContact==='nc'));
  }

  function likelySafetyInput(label, source) {
    const key=String(label||'').trim().toUpperCase();
    const role=metadataFunctionRole(key);
    ...
    const item=variableMetadata(key),metaText=item?[item.symbolicName,item.name,item.description].filter(Boolean).join(' '):'';
    const sourceText=String(source && (source.label || source.name || source.description || source.reference || '') || '');
    if(critical.test(String(label||'')+' '+sourceText+' '+metaText)) return true;
    const hmi=hmiInputElement(key);
    if(hmi){const text=[hmi.label,hmi.name,hmi.description,hmi.reference,metaText].filter(Boolean).join(' ');return hmi.type==='estop'||critical.test(text)||(hmi.type==='pushbutton'&&hmi.contactType==='nc'&&/STOP|PARADA/i.test(text));}
    return false;
  }
  ...
  function sortPLC(a, b) {
    ...
    let lNumber = lMatch ? parseInt(lMatch[0], 10) : 99999;
    let rNumber = rMatch ? parseInt(rMatch[0], 10) : 99999;
    if(global.SimuPLCPLCProfile){
      const la=global.SimuPLCPLCProfile.addressOrdinal(left), ra=global.SimuPLCPLCProfile.addressOrdinal(right);
      if(Number.isFinite(la)) lNumber=la;
      if(Number.isFinite(ra)) rNumber=ra;
    }
  ...
  function generateFBDESP32(state, pinMap) {
    ...
    const digitalInputSafetyFlags = digitalInputs.map(function (node, index) { return likelySafetyInput(digitalInputLabels[index], node); });
    ...
    nodes.forEach(function (node, index) {
      ...
      if (type === 'analog_constant') {
        ...
        a=num(analog.inMin!==undefined?analog.inMin:node.inMin,0);b=num(analog.inMax!==undefined?analog.inMax:node.inMax,100);c=num(analog.neutral!==undefined?analog.neutral:node.neutral,50);d=Math.max(0,num(analog.deadband!==undefined?analog.deadband:node.deadband,2));e=Math.max(0,num(analog.outMax!==undefined?analog.outMax:node.outMax,100));clampFlag=analog.clamp!==false?1:0;
    ...
    const connSrc = [], connSrcPin = [], connDst = [], connPin = [], connInv = [], connSignal = [];
    connections.forEach(function (connection) {
      ...
      connSrcPin.push(Math.max(0,integer(String(connection.fromPin||'out1').replace(/\D/g,''),1,1,255)-1));
    ...
    lines.push('const bool digitalInputIsSafety[SAFE_DIGITAL_INPUTS] = { ' + cppArray(digitalInputSafetyFlags, false, function (value) { return value ? 'true' : 'false'; }) + ' };');
    ...
    lines.push('  T_AI=40,T_SCALE=41,T_GT=42,T_LT=43,T_EQ=44,T_GTE=45,T_LTE=46,T_HYST=47,T_PWM=48,T_AO=49,T_CONST=50,T_PID=51,T_SPLIT=52');
    lines.push('};');
    lines.push('');
    ...
    lines.push('const uint8_t connSrcPin[SAFE_CONNS] = { ' + cppArray(connSrcPin, 0, String) + ' }; // 0=salida principal/LLENAR, 1=VACIAR');
    ...
    lines.push('float splitFill[SAFE_NODES] = {0};');
    lines.push('float splitDrain[SAFE_NODES] = {0};');
    ...
    lines.push('const uint32_t SIMUPLC_SESSION_TIMEOUT_MS = 6500UL;');
    lines.push('bool simuplcSessionSeen = false;');
    lines.push('bool simuplcRemoteModeArmed = false;');
    lines.push('bool simuplcLastSessionActive = false;');
    lines.push('uint32_t simuplcLastCommandAt = 0;');
    ...
    lines.push('bool hmiDigitalInputs[SAFE_DIGITAL_INPUTS] = { ' + cppArray(digitalInputNcFlags,false,function(value){return value?'true':'false';}) + ' };');
    ...
    lines.push('}');
    lines.push('float analogSourceValue(uint16_t source,uint8_t sourcePin){if(source>=N_NODES)return NAN;if(nodeType[source]==T_SPLIT)return sourcePin==1?splitDrain[source]:splitFill[source];return aValue[source];}');
    lines.push('float readAnalogPin(uint16_t node,uint8_t pin){');
    lines.push('  for(uint16_t k=0;k<N_CONNS;k++) if(connDst[k]==node && connPin[k]==pin && connSignal[k]==1) return analogSourceValue(connSrc[k],connSrcPin[k]);');
    ...
    lines.push('bool simuplcSessionActive(){return simuplcSessionSeen&&((uint32_t)(millis()-simuplcLastCommandAt)<=SIMUPLC_SESSION_TIMEOUT_MS);}');
    lines.push('uint8_t simuplcEffectiveControlMode(){return (simuplcSessionActive()&&simuplcRemoteModeArmed)?simuplcControlMode:SIMUPLC_PHYSICAL;}');
    lines.push('bool effectiveDigitalInput(uint8_t ix){if(ix>=N_DIGITAL_INPUTS)return false;uint8_t mode=simuplcEffectiveControlMode();if(mode==SIMUPLC_PHYSICAL)return physicalDigitalInputs[ix];if(mode==SIMUPLC_HMI){if(digitalInputIsSafety[ix]&&digitalInputIsNc[ix])return physicalDigitalInputs[ix]&&hmiDigitalInputs[ix];return hmiDigitalInputs[ix];}return digitalInputIsNc[ix]?(physicalDigitalInputs[ix]&&hmiDigitalInputs[ix]):(physicalDigitalInputs[ix]||hmiDigitalInputs[ix]);}');
    lines.push('float effectiveAnalogInput(uint8_t ix){if(ix>=N_ANALOG_INPUTS)return 0.0f;uint8_t mode=simuplcEffectiveControlMode();if(mode==SIMUPLC_PHYSICAL)return physicalAnalogInputs[ix];if(mode==SIMUPLC_HMI)return hmiAnalogValid[ix]?hmiAnalogInputs[ix]:0.0f;return hmiAnalogValid[ix]?hmiAnalogInputs[ix]:physicalAnalogInputs[ix];}');
    lines.push('const char* simuplcRequestedModeName(){return simuplcControlMode==SIMUPLC_HMI?"HMI":(simuplcControlMode==SIMUPLC_PHYSICAL?"PHYSICAL":"BOTH");}');
    lines.push('const char* simuplcModeName(){uint8_t mode=simuplcEffectiveControlMode();return mode==SIMUPLC_HMI?"HMI":(mode==SIMUPLC_PHYSICAL?"PHYSICAL":"BOTH");}');
    lines.push('void updateSimuPLCSession(){bool active=simuplcSessionActive();if(simuplcLastSessionActive&&!active){simuplcRemoteModeArmed=false;simuplcProtocolActive=false;simuplcRunning=true;simuplcScanRequested=true;simuplcStateRequested=false;for(uint8_t i=0;i<N_ANALOG_INPUTS;i++)hmiAnalogValid[i]=false;}simuplcLastSessionActive=active;}');
    lines.push('');
    ...
    lines.push('      else if(t==T_SPLIT){float input=readAnalogPin(n,0);if(invalidFloat(input)){splitFill[n]=0.0f;splitDrain[n]=0.0f;aValue[n]=nodeP3[n];}else{if(nodeClamp[n])input=clampFloat(input,nodeP1[n],nodeP2[n]);float half=max(0.0f,nodeP4[n])*0.5f;float low=clampFloat(nodeP3[n]-half,nodeP1[n],nodeP2[n]);float high=clampFloat(nodeP3[n]+half,nodeP1[n],nodeP2[n]);float fill=0.0f,drain=0.0f;if(input>high&&nodeP2[n]>high)fill=mapFloat(input,high,nodeP2[n],0.0f,nodeP5[n],true);else if(input<low&&low>nodeP1[n])drain=mapFloat(input,low,nodeP1[n],0.0f,nodeP5[n],true);splitFill[n]=clampFloat(fill,0.0f,nodeP5[n]);splitDrain[n]=clampFloat(drain,0.0f,nodeP5[n]);if(splitFill[n]>0.0f)splitDrain[n]=0.0f;else if(splitDrain[n]>0.0f)splitFill[n]=0.0f;aValue[n]=input;}}');
    ...
    lines.push('  Serial.print(F(",RUNNING,"));Serial.print(simuplcRunning?1:0);Serial.print(F(",CONTROL_MODE,"));Serial.print(simuplcModeName());Serial.print(F(",REQUESTED_MODE,"));Serial.print(simuplcRequestedModeName());Serial.print(F(",HMI_CONNECTED,"));Serial.println(simuplcSessionActive()?1:0);');
    ...
    lines.push('  char* save=nullptr;char* cmd=strtok_r(line,",",&save);if(!cmd)return;simuplcProtocolActive=true;simuplcSessionSeen=true;simuplcLastCommandAt=millis();');
    lines.push('  if(strcmp(cmd,"HELLO")==0){Serial.println(F("OK,SIMUPLC,READY_AUTONOMOUS_V24_1,1"));simuplcScanRequested=true;simuplcStateRequested=true;return;}');
    ...
    lines.push('  updateSimuPLCSession();');
    ...
    lines.push('  if(simuplcProtocolActive&&simuplcSessionActive()&&(simuplcStateRequested||(uint32_t)(now-simuplcLastStateAt)>=SIMUPLC_STATE_PERIOD_MS))sendSimuPLCState();');
    lines.push('}');
    return lines.join('\n');
  }
  ...
  function ladderWireType(wire, byId) {
    ...
    for (const endpoint of endpoints) {
      ...
      if (type === 'analog_constant' || type === 'analog_input' || type === 'scale' || type === 'pid' || type === 'split_range') return 'analog';
  ...
  function generateLadderESP32(model, pinMap) {
    ...
    function analogSourceFor(inputPin) {
      const endpoints = Array.from(network(inputPin, 'analog')).sort();
      for (const endpoint of endpoints) {
        const parts = endpointParts(endpoint);
        if (parts.side !== 'out' && parts.side !== 'fill' && parts.side !== 'drain') continue;
        const element = byId.get(parts.elementId);
        const type = cleanType(element && element.type);
        if (type === 'analog_constant' || type === 'analog_input' || type === 'scale' || type === 'pid' || type === 'split_range') return {element:element,side:parts.side};
      }
      return null;
    }
    function analogSourceElementFor(inputPin){const source=analogSourceFor(inputPin);return source&&source.element;}
    function analogExpressionFor(inputPin){const source=analogSourceFor(inputPin);if(!source)return '0.0f';const variable=variableFor(source.element);if(cleanType(source.element.type)==='split_range')return variable+(source.side==='drain'?'_drain':'_fill');return variable;}
    ...
    const processors = elements.filter(function (element) { const type = cleanType(element.type); return type === 'scale' || type === 'pid' || type === 'split_range' || type === 'pwm_output' || type === 'analog_output' || COMPARATOR_TYPES.has(type); });
    ...
    const digitalInputSafetyFlags = digitalInputLabels.map(function (label) { const el=elements.find(function (element) { return String(element.label || '').toUpperCase() === String(label).toUpperCase(); }); return likelySafetyInput(label, el); });
    const defaultControlMode = generatedControlMode();
    ...
    orderedProcessors.forEach(function (element) {
      const variable = variableFor(element);
      if(cleanType(element.type)==='pid'){lines.push('float '+variable+' = 0.0f;');lines.push('PIDState '+variable+'_state = {0.0f,0.0f,0,0.0f};');} else if(cleanType(element.type)==='split_range'){lines.push('float '+variable+' = 0.0f;');lines.push('float '+variable+'_fill = 0.0f;');lines.push('float '+variable+'_drain = 0.0f;');} else if (cleanType(element.type) === 'scale' || cleanType(element.type) === 'pwm_output' || cleanType(element.type) === 'analog_output') lines.push('float ' + variable + ' = 0.0f;');
    ...
    orderedProcessors.forEach(function (element) {
      const type = cleanType(element.type);
      const variable = variableFor(element);
      const input = analogExpressionFor(pinId(element, 'in'));
      if(type==='pid'){
        const pv=analogExpressionFor(pinId(element,'pv')),sp=analogExpressionFor(pinId(element,'sp'));
        ...
        const inMin=cFloat(element.inMin,0),inMax=cFloat(element.inMax,100),neutral=cFloat(element.neutral,50),deadband=cFloat(Math.max(0,num(element.deadband,2))),outMax=cFloat(Math.max(0,num(element.outMax,100)));
        lines.push('  '+variable+' = '+(element.clamp!==false?'clampFloat('+input+','+inMin+','+inMax+')':input)+';');
        lines.push('  {float half='+deadband+'*0.5f;float low=clampFloat('+neutral+'-half,'+inMin+','+inMax+');float high=clampFloat('+neutral+'+half,'+inMin+','+inMax+');'+variable+'_fill=0.0f;'+variable+'_drain=0.0f;if('+variable+'>high&&'+inMax+'>high)'+variable+'_fill=mapFloat('+variable+',high,'+inMax+',0.0f,'+outMax+',true);else if('+variable+'<low&&low>'+inMin+')'+variable+'_drain=mapFloat('+variable+',low,'+inMin+',0.0f,'+outMax+',true);'+variable+'_fill=clampFloat('+variable+'_fill,0.0f,'+outMax+');'+variable+'_drain=clampFloat('+variable+'_drain,0.0f,'+outMax+');if('+variable+'_fill>0.0f)'+variable+'_drain=0.0f;else if('+variable+'_drain>0.0f)'+variable+'_fill=0.0f;}');
      } else if (type === 'scale') {
