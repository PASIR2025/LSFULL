(function(){
  ...
  const VERSION='V24.1-AUTONOMOUS-SYNC';
  ...
  let lastRemoteResyncAt=0;
  let remoteMismatchCount=0;
  ...
  function modeDescription(mode){
    if(mode==='hmi')return 'Solo HMI: mandos y sensores de proceso usan la señal virtual. STOP, emergencia y protecciones físicas NC conservan prioridad. Si se desconecta el HMI, el PLC vuelve automáticamente a control físico.';
    if(mode==='physical')return 'Control físico: todas las entradas proceden del Arduino/ESP32. El HMI solo visualiza. Este es también el modo autónomo de respaldo cuando no hay HMI conectado.';
    return 'HMI + físico: NO usa OR y NC usa AND. Si el HMI se desconecta, el PLC continúa autónomo con las entradas físicas.';
  }
  ...
  function isNc(tag){
    ...
    if(elements.some(function(el){
      ...
      if(el.type==='tank'){
        const key=cleanTag(tag);
        if(cleanTag(el.tag)===key)return el.lowSensorContactType==='nc';
        if(cleanTag(el.tag2)===key)return el.highSensorContactType==='nc';
      }
      return el.type==='estop'||el.contactType==='nc'||/STOP|PARADA|EMERGENCIA|E-?STOP|GUARDAMOTOR|PROTECCI(?:O|Ó)N|T[EÉ]RMICO|SOBRECARGA|\bNC\b/i.test(String(el.label||''));
    }))return true;
  ...
  function variableMetaText(tag){
    try{
      const vm=window.SimuPLCVariableManager,meta=vm&&vm.getMetadata?vm.getMetadata():null,item=meta&&meta.variables&&meta.variables[cleanTag(tag)];
      return item?[item.symbolicName,item.name,item.description].filter(Boolean).join(' '):'';
    }catch(_){return '';}
  }
  function variableFunctionRole(tag){
    try{
      const vm=window.SimuPLCVariableManager,meta=vm&&vm.getMetadata?vm.getMetadata():null,item=meta&&meta.variables&&meta.variables[cleanTag(tag)];
      const role=String(item&&(item.functionRole||item.role||item.function)||'').trim().toLowerCase();
  ...
  function isSafety(tag){
    const key=cleanTag(tag),role=variableFunctionRole(key);
    if(role==='safety')return true;
    if(role==='process'||role==='command')return false;
    const critical=/STOP|PARADA|EMERGENCIA|E-?STOP|GUARDAMOTOR|PROTECCI(?:O|Ó)N|T[EÉ]RMICO|SOBRECARGA|FALLA|SAFETY/i;
    const elements=findElements(key);
    if(elements.some(function(el){
      if(!el)return false;
      const text=[el.label,el.name,el.description,el.reference,variableMetaText(key)].filter(Boolean).join(' ');
      if(el.type==='estop')return true;
      if(critical.test(text))return true;
      return el.type==='pushbutton'&&el.contactType==='nc'&&/STOP|PARADA/i.test(text);
    }))return true;
    const node=fbdInputNode(key);
    const nodeText=[node&&node.name,node&&node.label,node&&node.description,node&&node.reference,variableMetaText(key)].filter(Boolean).join(' ');
    return critical.test(nodeText);
  }
  ...
  function updateModeUi(){
    ...
    const badge=document.getElementById('hmiGlobalModeBadgeV21');if(badge){
      const io=window.SimuPLCExternalIO,connected=!!(io&&io.isConnected&&io.isConnected()),state=extState();
      if(!connected){badge.textContent='PLC: LOCAL FÍSICO';badge.dataset.sync='offline';badge.title='Sin HMI conectado: el PLC trabaja autónomamente con entradas físicas.';}
      else if(!(state&&state.ready)){badge.textContent='PLC: SINCRONIZANDO';badge.dataset.sync='pending';badge.title='Esperando estado del controlador.';}
      else{
        const active=VALID_MODES.includes(String(state.controlMode||'').toLowerCase())?String(state.controlMode).toLowerCase():'physical';
        badge.textContent='SEL '+modeCommand(mode)+' · PLC '+modeCommand(active);
        badge.dataset.sync=active===mode?'ok':'mismatch';
        badge.title='Modo seleccionado: '+modeLabel(mode)+' · modo efectivo informado por PLC: '+modeLabel(active);
      }
      badge.dataset.mode=mode;
    }
  }

  async function sendModeToController(mode,force){
  ...
  function ensureToolbarMode(){
    ...
    wrap.innerHTML='<span class="hmi-global-caption">CONTROL</span>'+
      '<button type="button" data-global-control-mode="hmi" title="Solo HMI: sensores de proceso virtuales; seguridad física NC conserva prioridad">HMI</button>'+
  ...
  function ensureCodegenGlobalUi(){
    ...
    if(!global){
      ...
      global.innerHTML='<label>Fuente para todas las entradas<select id="arduinoGlobalControlModeV21">'+
        '<option value="hmi">Solo HMI (proceso virtual + seguridad física)</option><option value="physical">Control físico / autónomo</option><option value="both">HMI + físico</option></select></label>'+
        '<p id="arduinoGlobalControlHelpV21"></p><small>El .ino funciona sin PC/celular/tablet. Sin sesión HMI activa vuelve automáticamente a FÍSICO. En Solo HMI, los sensores de proceso físicos se ignoran; STOP/emergencia/protecciones físicas NC mantienen prioridad.</small>';
    ...
    if(status)status.textContent='Control autónomo V24.1 activo · PLC funciona sin HMI · Solo HMI aísla sensores físicos de proceso · seguridad física conserva prioridad.';
    updateModeUi();
  }
  ...
  function upgradeGeneratedCode(source){
    ...
    if(code.includes('SIMUPLC HMI READY CODE V24_1'))return code;
    if(!/SIMUPLC HMI READY CODE V(?:12|15|16|17|18|20|24)/.test(code))return code;

    const tags=parseInputTags(code);
    const ncFlags=parseInputNcFlags(code,tags);
    const safetyFlags=tags.map(function(tag){return isSafety(tag);});
    const hmiDefaults=ncFlags.map(function(nc){return nc?'true':'false';});
    const defaultMode=getMode()==='hmi'?1:getMode()==='physical'?2:0;

    code=code
      .replace(/SIMUPLC HMI READY CODE V(?:12|15|16|17|18|20|24)/g,'SIMUPLC HMI READY CODE V24_1')
      .replace(/READY_CODE_V(?:12|15|16|17|18|20|24)/g,'READY_CODE_V24_1')
      .replace(/GENERADOR HMI V(?:12|15|16|17|18|20|24)/g,'GENERADOR HMI V24.1');
    ...
    code=code.replace(
      ...
      'const uint32_t HMI_SESSION_TIMEOUT_MS=6500UL;\n'+
      'uint8_t hmiControlMode=HMI_DEFAULT_CONTROL_MODE; // modo remoto solicitado\n'+
      'bool hmiControllerRunning=true; // PLC autónomo desde el arranque\n'+
      'bool hmiProtocolActive=false,hmiSessionSeen=false,hmiRemoteModeArmed=false,hmiLastSessionActive=false;\n'+
      'uint32_t hmiLastMessageMs=0,hmiLastStateMs=0;\n'+
      'bool hmiScanRequested=true;\n'+
      'bool hmiStateRequested=false;\n'+
      'bool hmiStateInitialized=false;\n'+
      'bool hmiLastControllerRunning=false;\n'+
      'uint8_t hmiLastControlMode=255;\n'+
      'bool hmiLastSessionConnected=false;\n'+
...
`bool hmiSessionActive(){return hmiSessionSeen&&((uint32_t)(millis()-hmiLastMessageMs)<=HMI_SESSION_TIMEOUT_MS);}
uint8_t hmiEffectiveControlMode(){return (hmiSessionActive()&&hmiRemoteModeArmed)?hmiControlMode:HMI_MODE_PHYSICAL;}
bool hmiCombineInput(uint8_t ix,bool physicalValue){
  ...
  uint8_t mode=hmiEffectiveControlMode();
  if(mode==HMI_MODE_HMI){
    if(hmiInputIsSafety[ix]&&hmiInputIsNc[ix])return physicalValue&&hmiInputValues[ix];
    return hmiInputValues[ix];
  }
  if(mode==HMI_MODE_PHYSICAL)return physicalValue;
  return hmiInputIsNc[ix]?(physicalValue&&hmiInputValues[ix]):(physicalValue||hmiInputValues[ix]);
    ...
    const stateRuntime=`const char* hmiRequestedControlModeName(){return hmiControlMode==HMI_MODE_HMI?"HMI":(hmiControlMode==HMI_MODE_PHYSICAL?"PHYSICAL":"BOTH");}
const char* hmiControlModeName(){uint8_t mode=hmiEffectiveControlMode();return mode==HMI_MODE_HMI?"HMI":(mode==HMI_MODE_PHYSICAL?"PHYSICAL":"BOTH");}
String hmiBuildState(){String s="STATE";for(uint8_t i=0;i<HMI_INPUT_COUNT;i++){s+=',';s+=HMI_INPUT_TAGS[i];s+=',';s+=(hmiLogicalInput(i)?'1':'0');s+=',';s+=HMI_INPUT_TAGS[i];s+="_PHYSICAL,";s+=(hmiPhysicalValues[i]?'1':'0');s+=',';s+=HMI_INPUT_TAGS[i];s+="_HMI,";s+=(hmiInputValues[i]?'1':'0');}for(uint8_t i=0;i<HMI_OUTPUT_COUNT;i++){s+=',';s+=HMI_OUTPUT_TAGS[i];s+=',';s+=(hmiLogicalOutput(i)?'1':'0');}s+=",RUNNING,";s+=(hmiControllerRunning?'1':'0');s+=",CONTROL_MODE,";s+=hmiControlModeName();s+=",REQUESTED_MODE,";s+=hmiRequestedControlModeName();s+=",HMI_CONNECTED,";s+=(hmiSessionActive()?'1':'0');return s;}
bool hmiStateChanged(){bool session=hmiSessionActive();uint8_t effective=hmiEffectiveControlMode();bool changed=!hmiStateInitialized||hmiLastControllerRunning!=hmiControllerRunning||hmiLastControlMode!=effective||hmiLastSessionConnected!=session;for(uint8_t i=0;i<HMI_INPUT_COUNT;i++){bool logical=hmiLogicalInput(i);if(hmiLastLogicalInputs[i]!=logical||hmiLastPhysicalInputs[i]!=hmiPhysicalValues[i]||hmiLastHmiInputs[i]!=hmiInputValues[i])changed=true;hmiLastLogicalInputs[i]=logical;hmiLastPhysicalInputs[i]=hmiPhysicalValues[i];hmiLastHmiInputs[i]=hmiInputValues[i];}for(uint8_t i=0;i<HMI_OUTPUT_COUNT;i++){bool logical=hmiLogicalOutput(i);if(hmiLastLogicalOutputs[i]!=logical)changed=true;hmiLastLogicalOutputs[i]=logical;}hmiLastControllerRunning=hmiControllerRunning;hmiLastControlMode=effective;hmiLastSessionConnected=session;hmiStateInitialized=true;return changed;}
void hmiSendState();
void hmiProcessCommand(String command){command.trim();if(!command.length())return;hmiLastMessageMs=millis();hmiSessionSeen=true;hmiProtocolActive=true;if(command=="PING"){hmiTransportSend("PONG");return;}if(command=="GET_STATE"){hmiScanRequested=true;hmiStateRequested=true;return;}if(command=="RUN,1"||command=="RUN"){hmiControllerRunning=true;hmiScanRequested=true;hmiStateRequested=true;return;}if(command=="STOP"||command=="RUN,0"){hmiControllerRunning=false;hmiForceOutputsOff();hmiStateRequested=true;hmiSendState();return;}if(command.startsWith("HELLO")){hmiTransportSend("OK,SIMUPLC,READY_CODE_V24_1,1");hmiScanRequested=true;hmiStateRequested=true;return;}if(command.startsWith("MODE,")){String value=command.substring(5);value.trim();value.toUpperCase();uint8_t next=hmiControlMode;if(value=="HMI")next=HMI_MODE_HMI;else if(value=="PHYSICAL"||value=="FISICO")next=HMI_MODE_PHYSICAL;else if(value=="BOTH"||value=="AMBOS")next=HMI_MODE_BOTH;hmiControlMode=next;hmiRemoteModeArmed=true;hmiTransportSend(String("OK,MODE,")+hmiRequestedControlModeName());hmiScanRequested=true;hmiStateRequested=true;return;}if(command.startsWith("SET,")){int p=command.indexOf(',',4);if(p>4){String tag=command.substring(4,p);int ix=hmiFindInput(tag);if(ix>=0){bool next=command.substring(p+1).toInt()!=0;if(hmiInputValues[ix]!=next){hmiInputValues[ix]=next;hmiStateRequested=true;}hmiScanRequested=true;}}return;}hmiTransportSend("ERROR,COMANDO_NO_RECONOCIDO");}
void hmiSendState(){if(!hmiProtocolActive||!hmiSessionActive())return;hmiTransportSend(hmiBuildState());hmiLastStateMs=millis();hmiStateRequested=false;hmiStateChanged();}`;
    ...
    code=code.replace(/void hmiBegin\(\)\{hmiLastMessageMs=millis\(\);hmiTransportBegin\(\);hmiSendState\(\);\}/,
      'void hmiBegin(){hmiLastMessageMs=0;hmiTransportBegin();hmiScanRequested=true;hmiStateRequested=false;}');
    code=code.replace(/void hmiLoop\(\)\{hmiTransportLoop\(\);if\(HMI_STOP_ON_TIMEOUT&&hmiControllerRunning&&\(uint32_t\)\(millis\(\)-hmiLastMessageMs\)>HMI_TIMEOUT_MS\)\{hmiControllerRunning=false;hmiForceOutputsOff\(\);\}\}/,
      'void hmiLoop(){hmiTransportLoop();bool session=hmiSessionActive();if(hmiLastSessionActive&&!session){hmiRemoteModeArmed=false;hmiProtocolActive=false;hmiControllerRunning=true;hmiScanRequested=true;hmiStateRequested=false;}hmiLastSessionActive=session;}');
    code=code.replace(/void hmiMaybeSendState\(\)\{uint32_t now=millis\(\);if\(\(uint32_t\)\(now-hmiLastStateMs\)>=HMI_STATE_PERIOD_MS\)\{hmiLastStateMs=now;hmiSendState\(\);\}\}/,
      'void hmiMaybeSendState(){if(!hmiProtocolActive||!hmiSessionActive())return;uint32_t now=millis();bool changed=hmiStateChanged();if(changed||hmiStateRequested||(uint32_t)(now-hmiLastStateMs)>=HMI_STATE_PERIOD_MS)hmiSendState();}');
  ...
  function reconcileControllerMode(detail){
    const io=window.SimuPLCExternalIO;
    if(!(io&&io.isConnected&&io.isConnected()&&io.isReady&&io.isReady()))return;
    const state=detail&&typeof detail==='object'?detail:extState();
    if(!(state&&state.ready))return;
    const selected=getMode(),remote=String(state.controlMode||'').toLowerCase();
    if(!VALID_MODES.includes(remote))return;
    if(remote===selected){remoteMismatchCount=0;updateModeUi();return;}
    const now=Date.now();remoteMismatchCount++;
    if(now-lastRemoteResyncAt<850)return;
    lastRemoteResyncAt=now;
    sendModeToController(selected,true).catch(function(){});
    setTimeout(function(){const x=window.SimuPLCExternalIO;try{x&&x.requestStateThrottled&&x.requestStateThrottled();}catch(_){}},300);
    updateModeUi();
  }
  ...
  function decorateControls(){
    ...
    project.elements.forEach(function(el){
      ...
      if(['pushbutton','sensor','photo-sensor','limit-sensor','universal-sensor','estop','selector'].includes(el.type)){
        ...
        badge.dataset.mode=mode;
        if(el.type==='selector'&&mode==='hmi'){
          const pos=Number(el.selectorPosition)||0,source={};
          if(el.tag)source[cleanTag(el.tag)]=pos===1;
          if(String(el.selectorType)==='3'&&el.tag2)source[cleanTag(el.tag2)]=pos===2;
          setSelectorVisual(node,el,null,source);
        }
      }
    });
  }
  ...
  function installStyles(){
    ...
    const style=document.createElement('style');style.id='simuplc-v21-styles';style.textContent=`
      ...
      #hmiGlobalModeBadgeV21{display:inline-flex;align-items:center;min-height:26px;padding:0 7px;border-radius:7px;background:#f8fafc;font:900 8px Arial;color:#075985;white-space:nowrap}
      #hmiGlobalModeBadgeV21[data-sync="ok"]{background:#dcfce7;color:#166534}
      #hmiGlobalModeBadgeV21[data-sync="mismatch"]{background:#fee2e2;color:#b91c1c}
      #hmiGlobalModeBadgeV21[data-sync="pending"]{background:#fef3c7;color:#92400e}
      #hmiGlobalModeBadgeV21[data-sync="offline"]{background:#e2e8f0;color:#475569}
  ...
  function startObservers(){
    ...
    window.addEventListener('simuplc-external-state',function(event){
      ...
      // La selección del usuario es la autoridad. Si el firmware informa otro modo
      // durante una sesión válida, V24.1 reenvía MODE para recuperar la autoridad solicitada.
      reconcileControllerMode(detail);
      updateModeUi();
      clearTimeout(decorateTimer);
    ...
    setInterval(function(){ensureToolbarMode();ensureCodegenGlobalUi();patchExternalApi();patchHmiApi();reconcileControllerMode(extState());},750);
  }

  function boot(){
    installStyles();
    if(!localStorage.getItem(MODE_KEY))localStorage.setItem(MODE_KEY,'both');
    const cfg=readJson(CFG_KEY,{});if(!cfg.globalControlMode)cfg.globalControlMode=getMode();cfg.statePeriodMs=250;cfg.version=25;writeJson(CFG_KEY,cfg);
