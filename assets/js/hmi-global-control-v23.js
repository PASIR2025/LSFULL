(function(){
  ...
  function findElement(tag){
    ...
    return projectElements().find(function(el){
      return el&&[el.tag,el.tag2,el.tag3,el.tag4].map(cleanTag).includes(key)&&['pushbutton','sensor','photo-sensor','limit-sensor','estop','selector','tank','cylinder','cylinder-v'].includes(el.type);
    })||null;
  }
  ...
  async function sendModeToController(mode){
    ...
    }catch(error){console.warn('[SimuPLC V43] No se pudo sincronizar el modo global:',error);}
  }
  ...
  function ensureCodegenGlobalUi(){
    ...
    if(status)status.textContent='HMI V23 activo · generador Arduino V23 · modo global integrado · arranque USB Android corregido.';
    updateModeUi();
  }
  ...
  function upgradeGeneratedCode(source){
    ...
    if(!/SIMUPLC HMI READY CODE V(?:12|15|16|17|18|20|21|22)/.test(code))return code;
    ...
    code=code
      .replace(/SIMUPLC HMI READY CODE V(?:12|15|16|17|18|20|21|22)/g,'SIMUPLC HMI READY CODE V23')
      .replace(/READY_CODE_V(?:12|15|16|17|18|20|21|22)/g,'READY_CODE_V23')
      .replace(/GENERADOR HMI V(?:12|15|16|17|18|20|21|22)/g,'GENERADOR HMI V23');
    ...
    code=code.replace(
      ...
      'bool hmiExplicitStop=false; // STOP debe mantenerse; HELLO/SET pueden arrancar si no hubo STOP explícito\n'+
      'uint32_t hmiLastMessageMs=0,hmiLastStateMs=0;\n'+
...
void hmiProcessCommand(String command){
  ...
  if(command=="RUN,1"||command=="RUN"){hmiExplicitStop=false;hmiControllerRunning=true;hmiScanRequested=true;hmiStateRequested=true;return;}
  if(command=="STOP"||command=="RUN,0"){hmiExplicitStop=true;hmiControllerRunning=false;hmiForceOutputsOff();hmiStateRequested=true;hmiSendState();return;}
  if(command.startsWith("HELLO")){if(!hmiExplicitStop)hmiControllerRunning=true;hmiTransportSend("OK,SIMUPLC,READY_CODE_V23,1");hmiScanRequested=true;hmiStateRequested=true;return;}
  if(command.startsWith("MODE,")){
    ...
    if(next!=hmiControlMode){hmiControlMode=next;if(!hmiExplicitStop)hmiControllerRunning=true;hmiScanRequested=true;hmiStateRequested=true;}else hmiStateRequested=true;
    return;
  }
  if(command.startsWith("SET,")){
    int p=command.indexOf(',',4);
    if(p>4){String tag=command.substring(4,p);int ix=hmiFindInput(tag);if(ix>=0){bool next=command.substring(p+1).toInt()!=0;if(hmiInputValues[ix]!=next){hmiInputValues[ix]=next;hmiStateRequested=true;}if(!hmiExplicitStop)hmiControllerRunning=true;hmiScanRequested=true;}}
    return;
  }
...
void hmiSendState(){hmiTransportSend(hmiBuildState());hmiLastStateMs=millis();hmiStateRequested=false;hmiStateChanged();}`;
  ...
  function patchGenerator(){
    ...
    window.__SimuPLCV23UpgradeGeneratedCode=upgradeGeneratedCode;
    return true;
  }
  ...
  function scheduleRegenerate(){
    ...
    regenTimer=setTimeout(function(){
      try{
        ...
        const ladder=document.body&&document.body.classList.contains('mode-ladder')||document.body&&document.body.classList.contains('mode-hmi')&&window.SimuPLCHMI&&window.SimuPLCHMI.getProject&&((window.SimuPLCHMI.getProject()||{}).source==='ladder');
        if(ladder&&typeof window.SimuPLCGenerateLadderArduinoNow==='function')window.SimuPLCGenerateLadderArduinoNow();
        else if(text&&typeof window.generateArduinoSketch==='function')text.value=window.generateArduinoSketch();
      }catch(_){}
    },90);
  }
  ...
  function patchExternalApi(){
    ...
    io.connect=async function(){
      ...
      const mode=getMode();
      if(mode!=='both')setTimeout(function(){sendModeToController(mode);},350);
      return result;
    };
    return true;
  }
  ...
  function startObservers(){
    ...
    window.addEventListener('simuplc-external-state',function(event){
      ...
      // La selección del usuario es la autoridad. Un firmware anterior puede responder
  ...
  function boot(){
    ...
    const cfg=readJson(CFG_KEY,{});if(!cfg.globalControlMode)cfg.globalControlMode=getMode();cfg.statePeriodMs=250;cfg.version=43;writeJson(CFG_KEY,cfg);
