(function (global) {
  ...
  async function maybeOfferRecovery() {
    ...
    try {
      ...
      writeRecoveryState(state);
      try {
        if (global.SimuPLCHMI && typeof global.SimuPLCHMI.discardStartupRecovery === 'function') global.SimuPLCHMI.discardStartupRecovery();
      } catch (_error) {}
      toast('Recuperación descartada · editores limpios');
      return false;
    } catch (error) {
      state.pending = false;
