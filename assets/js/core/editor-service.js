(function (global) {
  ...
  function inspectProject(source) {
    ...
    const result = { activeEditor: source.activeEditor || null, plcProfileId: source.plcProfileId || (source.hardware && source.hardware.plcProfileId) || (source.mcuConfig && source.mcuConfig.plcProfileId) || null, hasFbd: false, hasLadder: false, fbd: null, ladder: null, hardware: source.hardware || source.mcuConfig || null };
  ...
  async function loadProjectSelective(source) {
    ...
    if (project.plcProfileId && global.SimuPLCPLCProfile && typeof global.SimuPLCPLCProfile.set === 'function') {
      global.SimuPLCPLCProfile.set(project.plcProfileId, { source: 'editor-project-load', migrate: false, migrateStorage: false });
    }
    let ok = true;
