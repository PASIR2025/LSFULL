(function (global) {
  ...
  function createProject(options) {
    ...
    return {
      ...
      plcProfileId: String(options.plcProfileId || (isObject(options.hardware) && options.hardware.plcProfileId) || (global.SimuPLCPLCProfile && global.SimuPLCPLCProfile.getId ? global.SimuPLCPLCProfile.getId() : 'classic')),
      editors: {
        fbd: normalizeFBD(options.fbd || emptyFBD()),
        ladder: normalizeLadder(options.ladder || emptyLadder())
      },
      hardware: (function(){
        const hw=isObject(options.hardware) ? clone(options.hardware) : {};
        if(!hw.plcProfileId) hw.plcProfileId=String(options.plcProfileId || (global.SimuPLCPLCProfile && global.SimuPLCPLCProfile.getId ? global.SimuPLCPLCProfile.getId() : 'classic'));
        return hw;
      })()
    };
  }
  ...
  function migrate(input, fallbacks) {
    ...
    if (source.type === config.projectType && source.editors) {
      return createProject({
        ...
        plcProfileId: source.plcProfileId || (source.hardware && source.hardware.plcProfileId) || (source.mcuConfig && source.mcuConfig.plcProfileId),
    ...
    if (source.editors || source.fbd || source.ladder || source.type === config.legacyDualProjectType) {
      return createProject({
        ...
        createdAt: source.createdAt || source.savedAt,
        activeEditor: source.activeEditor,
        plcProfileId: source.plcProfileId || (source.hardware && source.hardware.plcProfileId) || (source.mcuConfig && source.mcuConfig.plcProfileId) || (fallbacks.hardware && fallbacks.hardware.plcProfileId),
    ...
    if (Array.isArray(source.nodes) || (source.data && Array.isArray(source.data.nodes))) {
      return createProject({
        ...
        activeEditor: 'fbd',
        plcProfileId: source.plcProfileId || (source.hardware && source.hardware.plcProfileId) || (fallbacks.hardware && fallbacks.hardware.plcProfileId),
  ...
  function toLegacyDual(project) {
    ...
    return {
      ...
      plcProfileId: project.plcProfileId || (project.hardware && project.hardware.plcProfileId) || 'classic',
