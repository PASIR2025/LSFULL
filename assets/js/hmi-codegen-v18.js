(function(){
  ...
  function getStoredCfg(){ const cfg=Object.assign({},defaults,readJson(CFG_KEY,defaults)); if(!cfg.version||cfg.version<17){if(cfg.transport==='none')cfg.transport='serial';cfg.version=17;writeJson(CFG_KEY,cfg);} return cfg; }
  function getCfg(){
    ...
    if(q('arduinoHmiTimeout'))cfg.timeoutMs=number(q('arduinoHmiTimeout').value,3000,500,600000);
    if(q('arduinoHmiPeriod'))cfg.statePeriodMs=number(q('arduinoHmiPeriod').value,250,40,5000);
  ...
  function ensureUi(){
    ...
    const style=document.createElement('style');style.id='arduino-hmi-codegen-v17-style';style.textContent=`
    ...
    const box=document.createElement('div');box.id='arduinoHmiBox';box.innerHTML=`
      ...
      <div class="sub2">El archivo generado incluirá tu circuito, USB/OTG o Wi‑Fi, pines físicos y protocolo HMI. Verifica que el código muestre “SIMUPLC HMI READY CODE V17”.</div>
...
void hmiTransportSend(const String& line){
  if(hmiTxOffset<hmiTxBuffer.length()){
    if(line.startsWith("STATE"))hmiTxStatePending=true;
    return;
  }
  hmiTxBuffer=line;hmiTxBuffer+='\\n';hmiTxOffset=0;
}
void hmiTransportFlush(){
  while(hmiTxOffset<hmiTxBuffer.length() && Serial.availableForWrite()>0){
    int room=Serial.availableForWrite();
    if(room<=0)break;
    uint16_t left=(uint16_t)(hmiTxBuffer.length()-hmiTxOffset);
    uint16_t count=(uint16_t)min((int)left,room);
    Serial.write((const uint8_t*)hmiTxBuffer.c_str()+hmiTxOffset,count);
    hmiTxOffset+=count;
  }
  if(hmiTxBuffer.length() && hmiTxOffset>=hmiTxBuffer.length()){
    hmiTxBuffer="";hmiTxOffset=0;
    if(hmiTxStatePending){hmiTxStatePending=false;hmiTransportSend(hmiBuildState());}
  }
}
void hmiTransportBegin(){ Serial.begin(HMI_BAUD_RATE);hmiTxBuffer.reserve(256); }
...
`:buildLadderHelpers(labels);
    return `
// ===== SIMUPLC HMI READY CODE V18 - USB ANDROID NO BLOQUEANTE =====
const uint32_t HMI_BAUD_RATE=${Math.trunc(cfg.baudRate)}UL;
const uint32_t HMI_STATE_PERIOD_MS=${Math.trunc(cfg.statePeriodMs)}UL;
const uint32_t HMI_TIMEOUT_MS=${Math.trunc(cfg.timeoutMs)}UL;
...
bool hmiInputValues[HMI_SAFE_INPUT_COUNT]={${hmiDefaults(labels)}};
bool hmiPhysicalValues[HMI_SAFE_INPUT_COUNT]={false};
const uint8_t hmiInputSource[HMI_SAFE_INPUT_COUNT]={${hmiModes(labels)}}; // 0 fisica, 1 HMI, 2 OR/cualquiera, 3 AND/ambos obligatorios
${hmiModeComments(labels)}
...
// ===== FIN SIMUPLC HMI READY CODE V18 =====
`;
  }
  ...
  function wrapFbd(base,labels,cfg,board){
    if(!base||base.includes('SIMUPLC HMI READY CODE V17')||base.includes('SIMUPLC HMI READY CODE V16')||base.includes('SIMUPLC HMI READY CODE V15')||base.includes('SIMUPLC HMI READY CODE V12'))return base;
  ...
  function patchGenerator(){
    ...
    window.generateArduinoSketch=function(){
      ...
      if(!ready.includes('SIMUPLC HMI READY CODE V17')){
        return '#error GENERADOR HMI V17 NO INTEGRADO. Selecciona USB / OTG y pulsa Regenerar.\n'+ready;
      }
      return ready;
    };
    window.SimuPLCWrapGeneratedSketch=wrapGenerated;
    window.SimuPLCGetCodegenConfig=()=>clone(getCfg());
    window.SimuPLCCodegenVersion='V17';
    return true;
  }
  ...
  function boot(){
    ...
    if(st)st.textContent='Generador HMI V17 activo · USB/OTG y Wi‑Fi listos. El código debe incluir SIMUPLC HMI READY CODE V17.';
  }
