(function(){
  ...
  function ensureUi(){
    ...
    const box=document.createElement('div');box.id='arduinoHmiBox';box.innerHTML=`
      ...
      <div class="sub2">El archivo generado incluirá tu circuito, USB/OTG o Wi‑Fi, pines físicos y protocolo HMI. Verifica que el código muestre “SIMUPLC HMI READY CODE V18”.</div>
...
`;
  ...
  function wrapFbd(base,labels,cfg,board){
    if(!base||base.includes('SIMUPLC HMI READY CODE V'))return base;
  ...
  function patchGenerator(){
    ...
    window.generateArduinoSketch=function(){
      ...
      if(!ready.includes('SIMUPLC HMI READY CODE V18')){
        console.error('[SimuPLC Codegen V18] No se pudo integrar la comunicación HMI en el código FBD.');
        return ready;
      }
      return ready;
    };
    ...
    window.SimuPLCCodegenVersion='V18';
    return true;
  }
  ...
  function boot(){
    ...
    if(st)st.textContent='Generador HMI V18 activo · USB/OTG y Wi‑Fi listos. El código debe incluir SIMUPLC HMI READY CODE V18.';
  }
