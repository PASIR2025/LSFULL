  ...
  class WebUsbSerialPort {
    ...
    async open(options) {
      ...
      try {
        ...
        if (this.inEndpoint && typeof this.device.clearHalt === 'function') {
          await this.device.clearHalt('in', this.inEndpoint.endpointNumber).catch(() => {});
        }
        if (this.outEndpoint && typeof this.device.clearHalt === 'function') {
          await this.device.clearHalt('out', this.outEndpoint.endpointNumber).catch(() => {});
        }
        this._createStreams();
        this.opened = true;
      } catch (error) {
    ...
    _createStreams() {
      const port = this;
      // Android WebUSB CDC es más estable con una sola transferencia IN por vez,
      // usando el tamaño real del endpoint y sin llenar una cola de 512 bytes.
      const readSize = Math.max(8, Number(this.packetSize) || 64);
      let readInFlight = false;

      async function transferInSafe() {
        try {
          const result = await port.device.transferIn(port.inEndpoint.endpointNumber, readSize);
          await ensureOk(result, 'Lectura USB');
          return result;
        } catch (error) {
          if (port._closing) throw error;
          if (typeof port.device.clearHalt === 'function') {
            await port.device.clearHalt('in', port.inEndpoint.endpointNumber).catch(() => {});
            const retry = await port.device.transferIn(port.inEndpoint.endpointNumber, readSize);
            await ensureOk(retry, 'Lectura USB');
            return retry;
          }
          throw error;
        }
      ...
      async function transferOutSafe(bytes) {
        try {
          ...
          if (port._closing) throw error;
          if (typeof port.device.clearHalt === 'function') {
            await port.device.clearHalt('out', port.outEndpoint.endpointNumber).catch(() => {});
            const retry = await port.device.transferOut(port.outEndpoint.endpointNumber, bytes);
            await ensureOk(retry, 'Escritura USB');
            return retry;
          }
          throw error;
        }
      ...
      this.readable = new ReadableStream({
        async pull(controller) {
          if (readInFlight || port._closing) return;
          readInFlight = true;
          try {
            const result = await transferInSafe();
      ...
      this.writable = new WritableStream({
        async write(chunk) {
          ...
          // Los comandos SimuPLC son cortos; se dividen por paquete para evitar
          // transferencias largas que algunos Android CDC dejan pendientes.
          const packet = Math.max(8, Number(port.outEndpoint.packetSize) || 64);
          for (let offset = 0; offset < bytes.byteLength; offset += packet) {
            await transferOutSafe(bytes.subarray(offset, Math.min(bytes.byteLength, offset + packet)));
          }
        },
        close() {},
        abort() {}
