...
(function (global) {
  ...
  class WebUsbSerialPort {
    ...
    _createStreams() {
      const port = this;
      // En Android, leer exactamente uno o pocos paquetes USB evita que CDC-ACM quede
      // aparentemente abierto pero sin entregar las respuestas STATE del Arduino.
      const requested = Math.max(port.packetSize, Number(port.options.bufferSize) || 128);
      const readSize = port.driver === 'CDC-ACM'
        ? port.packetSize
        : Math.min(requested, port.packetSize * 4);
      ...
      this.writable = new WritableStream({
        async write(chunk) {
          ...
          const packet = Math.max(8, Number(port.outEndpoint.packetSize) || port.packetSize || 64);
          for (let offset = 0; offset < bytes.byteLength; offset += packet) {
            await port._writePacket(bytes.slice(offset, Math.min(bytes.byteLength, offset + packet)));
          }
        },
        close() {},
        abort() {}
