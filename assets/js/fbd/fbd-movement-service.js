(function(global){
  ...
  function bind(el,node){
    ...
    el.addEventListener('mousedown',function(event){
      ...
      if(adapter.getSimulation()) return;
      if(adapter.getSelectedNode()!==node){
        try{
          var selection=global.SimuPLCFBDSelection;
          if(selection&&typeof selection.selectNode==='function') selection.selectNode(node);
          else if(typeof global.selectNode==='function') global.selectNode(node);
        }catch(_e){}
      }
      if(adapter.getSelectedNode()!==node) return;
