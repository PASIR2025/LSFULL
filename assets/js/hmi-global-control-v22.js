(function(){
  ...
  async function sendModeToController(mode){
    ...
    }catch(error){console.warn('[SimuPLC V25] No se pudo sincronizar el modo global:',error);}
  }
  ...
  function ensureCodegenGlobalUi(){
    ...
    if(status)status.textContent='HMI V22 activo · generador Arduino V21 compatible · modo global · comunicación USB más estable.';
    updateModeUi();
  }
...
void hmiSendState(){hmiTransportSend(hmiBuildState());hmiLastStateMs=millis();hmiStateRequested=false;hmiStateChanged();}`;
  ...
  function boot(){
    ...
    const cfg=readJson(CFG_KEY,{});if(!cfg.globalControlMode)cfg.globalControlMode=getMode();cfg.statePeriodMs=1000;cfg.version=40;writeJson(CFG_KEY,cfg);
