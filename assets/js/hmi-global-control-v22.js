(function(){
  ...
  async function sendModeToController(mode){
    ...
    try{
      ...
      modeSyncTimer=setTimeout(function(){try{io.requestState&&io.requestState();}catch(_){}},180);
  ...
  function ensureCodegenGlobalUi(){
    ...
    if(period){period.value='120';period.disabled=true;}
...
void hmiSendState(){hmiTransportSend(hmiBuildState());hmiLastStateMs=millis();hmiStateRequested=false;hmiStateChanged();}`;
  ...
  function boot(){
    ...
    const cfg=readJson(CFG_KEY,{});if(!cfg.globalControlMode)cfg.globalControlMode=getMode();cfg.statePeriodMs=120;cfg.version=21;writeJson(CFG_KEY,cfg);
