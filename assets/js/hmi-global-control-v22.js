(function(){
  ...
  async function sendModeToController(mode){
    ...
    try{
      if(io.sendControlMode)await io.sendControlMode(mode);else await io.sendCommand('MODE,'+command);
      clearTimeout(modeSyncTimer);
      modeSyncTimer=setTimeout(function(){try{io.requestStateThrottled&&io.requestStateThrottled();}catch(_){}},1200);
  ...
  function ensureCodegenGlobalUi(){
    ...
    if(!global){
      ...
      global.innerHTML='<label>Modo inicial del Arduino<select id="arduinoGlobalControlModeV21">'+
        ...
        '<p id="arduinoGlobalControlHelpV21"></p><small>Este es solo el modo de arranque. Durante la operación puedes cambiar HMI, FÍSICO o AMBOS desde la barra superior.</small>';
    ...
    if(sub)sub.textContent='El código generado soporta siempre HMI, FÍSICO y AMBOS. Aquí solo eliges el modo con el que inicia el Arduino.';
    const period=document.getElementById('arduinoHmiPeriod');
    if(period){period.value='1000';period.disabled=true;}
    const status=document.getElementById('arduinoHmiStatus');
    if(status)status.textContent='HMI V43 activo · tres modos en tiempo real · ACK de comandos · estado por cambios.';
    updateModeUi();
  }
  ...
  function upgradeGeneratedCode(source){
    ...
    if(code.includes('SIMUPLC HMI READY CODE V43'))return code;
    if(code.includes('SIMUPLC HMI READY CODE V42'))return code.replace(/V42/g,'V43');
    ...
    code=code
      .replace(/SIMUPLC HMI READY CODE V(?:12|15|16|17|18|20)/g,'SIMUPLC HMI READY CODE V21')
      .replace(/READY_CODE_V(?:12|15|16|17|18|20)/g,'READY_CODE_V21')
      .replace(/GENERADOR HMI V(?:12|15|16|17|18|20)/g,'GENERADOR HMI V21');

    code=code.replace(/const uint32_t HMI_STATE_PERIOD_MS=\d+UL;/,'const uint32_t HMI_STATE_PERIOD_MS=1000UL;');
...
void hmiSendState(){hmiTransportSend(hmiBuildState());hmiLastStateMs=millis();hmiStateRequested=false;hmiStateChanged();}`;
  ...
  function boot(){
    ...
    const cfg=readJson(CFG_KEY,{});if(!cfg.globalControlMode)cfg.globalControlMode=getMode();cfg.statePeriodMs=1000;cfg.version=42;writeJson(CFG_KEY,cfg);
