/*
 * SimuPLC WebUSB Serial V20
 * Puerto serial USB robusto para Android mediante WebUSB.
...
(function (global) {
  ...
  class WebUsbSerialPort {
    ...
    async _claim(interfaceNumber, alternateSetting) {
      if (!this._claimed.includes(interfaceNumber)) {
        await this.device.claimInterface(interfaceNumber);
        this._claimed.push(interfaceNumber);
      }
      if (alternateSetting && typeof this.device.selectAlternateInterface === 'function') {
    ...
    async _configureDriver() {
      ...
      if (this.driver === 'GENERIC-BULK') return this._openGenericBulk();
    ...
    async _openCdc() {
      ...
      // Algunos Arduino y placas compatibles exponen únicamente una interfaz bulk.
      // En Android se puede usar esa interfaz aunque los descriptores CDC estén incompletos.
      await this._openGenericBulk();
    }

    async _openGenericBulk() {
      ...
      this.controlInterfaceNumber = io.iface.interfaceNumber;
      try { await this.setSignals({ dataTerminalReady: true, requestToSend: true }); } catch (_) {}
    }
    ...
    _createStreams() {
      const port = this;
      const readSize = Math.max(this.packetSize, Number(this.options.bufferSize) || 512);
      this.readable = new ReadableStream({
        type: 'bytes',
        async pull(controller) {
          try {
            const result = await port.device.transferIn(port.inEndpoint.endpointNumber, readSize);
            await ensureOk(result, 'Lectura USB');
      ...
      this.writable = new WritableStream({
        async write(chunk) {
          ...
          await ensureOk(result, 'Escritura USB');
        },
        close() {},
        abort() {}
      });
  ...
  function hasBulkDescriptors(device) {
    const configs = Array.isArray(device && device.configurations) ? device.configurations : [];
    return configs.some((configuration) => {
      try { findBulkInterface(configuration); return true; } catch (_) { return false; }
    });
  }
  ...
  function assertUsbUserGesture() {
    const activation = navigator.userActivation;
    if (activation && activation.isActive === false) {
      throw new DOMException('Toca nuevamente “Conectar” para autorizar el USB. Android exige que el selector se abra directamente desde un toque.', 'SecurityError');
    }
  }
  ...
  function detectDriver(device) {
    ...
    if ([0x2341, 0x2A03, 0x239A, 0x1B4F, 0x303A, 0x2E8A, 0x0483, 0x16C0, 0x04D8, 0x1209, 0x1366].includes(vendor)) {
      return 'CDC-ACM';
    }
    if (hasBulkDescriptors(device)) return 'GENERIC-BULK';
    return 'CDC-ACM';
  }
  ...
  function isSupportedDevice(device) {
    ...
    const vendor = Number(device.vendorId);
    if ([0x1A86, 0x10C4, 0x0403, 0x2341, 0x2A03, 0x239A, 0x1B4F, 0x303A, 0x2E8A, 0x0483, 0x16C0, 0x04D8, 0x1209, 0x1366].includes(vendor)) return true;
    return hasCdcDescriptors(device) || hasBulkDescriptors(device);
  }
  ...
  async function requestPort() {
    ...
    if (!navigator.usb || typeof navigator.usb.requestDevice !== 'function') {
      throw new Error('Este contenedor no ofrece WebUSB. Abre la misma dirección en Google Chrome para conectar el Arduino por OTG.');
    }
    assertUsbUserGesture();
    let device;
    try {
      if (isAndroid()) {
        // Debe ejecutarse inmediatamente dentro del gesto del usuario. Consultar
        // getDevices() antes del selector puede hacer que algunos Android pierdan
        // la activación transitoria y bloqueen requestDevice().
        ...
        if (compatible.length === 1) device = compatible[0];
        else device = await navigator.usb.requestDevice({ filters: CHIP_FILTERS });
      }
    } catch (error) {
      const cause = asError(error);
      if (cause.name === 'NotFoundError') {
        throw new DOMException(
          'No se seleccionó una placa USB. Conecta el Arduino mediante OTG, acepta el permiso de Android, cierra ArduinoDroid/Serial USB Terminal y vuelve a tocar Conectar.',
          'NotFoundError'
        );
      }
      if (cause.name === 'SecurityError') {
        throw new DOMException('Android bloqueó el permiso USB. Toca Conectar nuevamente desde esta pantalla y acepta el dispositivo.', 'SecurityError');
      }
      throw cause;
    }
  ...
  function diagnostics() {
    ...
    return {
      ...
      drivers: ['CDC-ACM', 'CH340/CH341', 'CP210x', 'FTDI', 'GENERIC-BULK'],
      version: 20
    };
  }
