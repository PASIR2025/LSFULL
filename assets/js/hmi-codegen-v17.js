(function(){
  ...
  function ensureUi(){
    ...
    const box=document.createElement('div');box.id='arduinoHmiBox';box.innerHTML=`
      ...
      <div class="sub2">El archivo generado incluirá tu circuito, USB/OTG o Wi‑Fi, pines físicos y protocolo HMI. Verifica que el código muestre “SIMUPLC HMI READY CODE V42”.</div>
...
void hmiProcessCommand(String command){
  ...
  if(command.startsWith("HELLO")){hmiTransportSend("OK,SIMUPLC,READY_CODE_V42,1");hmiStateRequested=true;return;}
...
// ===== FIN SIMUPLC HMI READY CODE V42 =====
`;
  }
  ...
  function wrapFbd(base,labels,cfg,board){
    if(!base||base.includes('SIMUPLC HMI READY CODE V42'))return base;
  ...
  function patchGenerator(){
    ...
    window.generateArduinoSketch=function(){
      ...
      if(!ready.includes('SIMUPLC HMI READY CODE V42')){
        return '#error GENERADOR HMI V42 NO INTEGRADO. Selecciona USB / OTG y pulsa Regenerar.\n'+ready;
      }
      return ready;
    };
    window.SimuPLCWrapGeneratedSketch=wrapGenerated;
    window.SimuPLCGetCodegenConfig=()=>clone(getCfg());
    window.SimuPLCCodegenVersion='V42';
    return true;
  }
  ...
  function boot(){
    ...
    if(st)st.textContent='Generador HMI V42 activo · ACK de comandos · HMI/FÍSICO/AMBOS en tiempo real.';
  }
