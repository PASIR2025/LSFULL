(function(){
  ...
  function getCfg(){
    ...
    if(q('arduinoHmiPeriod'))cfg.statePeriodMs=number(q('arduinoHmiPeriod').value,120,40,5000);
  ...
  function hmiElementForTag(tag){
    try{
      ...
      return els.find(el=>[el.tag,el.tag2].map(cleanTag).includes(cleanTag(tag)))||null;
    }catch(_){return null;}
  }
